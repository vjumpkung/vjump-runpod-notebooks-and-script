...
# vae + text encoders are only used during caching (latents / text outputs are pre-cached).
dit="./diffusion_models/mp_rank_00_model_states.pt" # HunyuanVideo DiT
vae="./vae/pytorch_model.pt" # HunyuanVideo VAE
...
learning_rate=2e-4 # HunyuanVideo example uses 2e-4
...
# fp8 for DiT - strongly recommended for HunyuanVideo unless you have 24GB+ VRAM
fp8_base=true
# fp8_llm=true # run LLaMA text encoder in fp8 during caching (needed for < 16GB VRAM)

# reduce vram usage (max blocks_to_swap=36)
blocks_to_swap=0

# VAE memory settings (used during latent caching)
vae_chunk_size=32
vae_tiling=true
...
discrete_flow_shift=7.0 # HunyuanVideo standard (lower toward 3.0 for more detail)
...
# attention (select sdpa, flash_attn or xformers; add split_attn for xformers)
sdpa=true
# flash_attn=true
# xformers=true
# split_attn=true
...
# optional VAE tiling + fp8 LLaMA flags for caching
vae_cache_extra=""
[[ "$vae_tiling" == true ]] && vae_cache_extra="--vae_tiling"
te_cache_extra=""
[[ "$fp8_llm" == true ]] && te_cache_extra="--fp8_llm"

python ${MUSUBI_TUNER_PATH}/cache_latents.py --dataset_config $dataset_config --vae $vae --vae_chunk_size $vae_chunk_size $vae_cache_extra
python ${MUSUBI_TUNER_PATH}/cache_text_encoder_outputs.py --dataset_config $dataset_config --text_encoder1 $text_encoder1 --text_encoder2 $text_encoder2 --batch_size $cache_text_encoder_batch_size $te_cache_extra
accelerate launch --dynamo_backend no --dynamo_mode default --mixed_precision $mixed_precision --num_processes 1 --num_machines 1 --num_cpu_threads_per_process 2 ${MUSUBI_TUNER_PATH}/hv_train_network.py ${args[@]}
