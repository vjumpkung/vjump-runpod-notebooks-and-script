...
dit="./diffusion_models/flux1-dev-kontext_fp8_scaled.safetensors" # flux.1 kontext dev DiT (fp8 scaled)
vae="./vae/ae.safetensors" # flux ae vae
text_encoder1="./text_encoders/t5xxl_fp16.safetensors" # T5-XXL
...
# the downloaded DiT is fp8 scaled, so fp8 is enabled by default (keep both on)
fp8_base=true
fp8_scaled=true
# fp8_t5=true # run T5 text encoder in fp8 for extra VRAM savings

# reduce vram usage blocks_to_swap=16 for VRAM 24 GB blocks_to_swap=35 for VRAM 16 GB
blocks_to_swap=0

# advanced settings (If you know what is this then uncomment in each args)
# save_state=true
timestep_sampling="flux_shift" # recommended for FLUX.1 Kontext
...
python ${MUSUBI_TUNER_PATH}/flux_kontext_cache_text_encoder_outputs.py --dataset_config $dataset_config --text_encoder1 $text_encoder1 --text_encoder2 $text_encoder2 --batch_size $cache_text_encoder_batch_size
