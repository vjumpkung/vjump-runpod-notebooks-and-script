...
# Wan 2.1 ONE-FRAME (single-frame) training.
# Uses the standard Wan entry scripts with --one_frame on BOTH the latent cache
# step and training. Uses the Wan2.1 I2V-14B (or FLF2V-14B) weights + the shared
# Wan2.1 VAE and T5. Dataset must set fp_1f_clean_indices / fp_1f_target_index
# (see docs/wan_1f.md). If you change one-frame settings, re-cache WITHOUT
# --skip_existing so the cache is overwritten.
...
# task: i2v-14B (basic one-frame) or flf2v-14B (intermediate-frame one-frame)
task="i2v-14B"
...
# one-frame must be set on the latent cache too
of_cache_extra=""
[[ "$one_frame" == true ]] && of_cache_extra="--one_frame"

python ${MUSUBI_TUNER_PATH}/wan_cache_latents.py --dataset_config $dataset_config --vae $vae $of_cache_extra
