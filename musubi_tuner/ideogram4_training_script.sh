...
# NOTE: Ideogram 4 ships fp8 weights only. The DiT is ALWAYS fp8 (do NOT set --fp8_base).
dit="./diffusion_models/ideogram4_fp8_scaled.safetensors" # Ideogram 4 conditional DiT (fp8)
vae="./vae/flux2-vae.safetensors" # Flux2 KL-VAE (used for caching / sampling)
text_encoder="./text_encoders/qwen3vl_8b_fp8_scaled.safetensors" # Qwen3-VL 8B (used for caching / sampling)
...
# NOTE: --fp8_base is NOT used for Ideogram 4 (the base DiT is permanently fp8 and
#       is dequantized on-the-fly). Do not enable it.

# reduce vram usage (max blocks_to_swap=33)
blocks_to_swap=0

# advanced settings (If you know what is this then uncomment in each args)
# save_state=true
timestep_sampling="ideogram4_shift" # resolution-aware logit-normal sampler (default)
weighting_scheme="none" # Ideogram 4 only accepts "none" (plain MSE flow-matching)
gradient_checkpointing=true
seed=42
persistent_data_loader_workers=true
max_data_loader_n_workers=2 # for faster dataset loading

# attention (select sdpa, flash_attn or xformers)
sdpa=true
# flash_attn=true
# xformers=true
...
python ${MUSUBI_TUNER_PATH}/ideogram4_cache_latents.py --dataset_config $dataset_config --vae $vae --vae_dtype bfloat16
python ${MUSUBI_TUNER_PATH}/ideogram4_cache_text_encoder_outputs.py --dataset_config $dataset_config --text_encoder $text_encoder --text_cache_dtype bf16
accelerate launch --dynamo_backend no --dynamo_mode default --mixed_precision $mixed_precision --num_processes 1 --num_machines 1 --num_cpu_threads_per_process 2 ${MUSUBI_TUNER_PATH}/ideogram4_train_network.py ${args[@]}
