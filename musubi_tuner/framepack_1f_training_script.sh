...
# FramePack ONE-FRAME (single-frame / kisekaeichi) training.
# Same as framepack_training_script.sh but with --one_frame enabled on BOTH the
# latent cache step and training. Dataset must provide start images via
# control_directory (see docs/framepack_1f.md). If you change one-frame settings,
# re-cache WITHOUT --skip_existing so the cache is overwritten.
...
# one-frame training (REQUIRED for this mode)
one_frame=true
# match these to your inference settings if you use no_2x / no_4x
...
# one-frame cache flags (must match the training flags)
of_cache_extra=""
[[ "$one_frame" == true ]] && of_cache_extra="--one_frame"
[[ "$one_frame_no_2x" == true ]] && of_cache_extra="$of_cache_extra --one_frame_no_2x"
[[ "$one_frame_no_4x" == true ]] && of_cache_extra="$of_cache_extra --one_frame_no_4x"
te_cache_extra=""
[[ "$fp8_llm" == true ]] && te_cache_extra="--fp8_llm"

python ${MUSUBI_TUNER_PATH}/fpack_cache_latents.py --dataset_config $dataset_config --vae $vae --image_encoder $image_encoder --vae_chunk_size $vae_chunk_size $of_cache_extra
