...
dit="./diffusion_models/z_image_turbo_bf16.safetensors" # z image turbo
vae="./vae/ae.safetensors" # ae vae 
...
# attention (select sdpa or xformers) (xformers is faster 20-30% but error)
sdpa=true
# xformers=true 
...
python ${MUSUBI_TUNER_PATH}/zimage_cache_latents.py --dataset_config $dataset_config --vae $vae
python ${MUSUBI_TUNER_PATH}/zimage_cache_text_encoder_outputs.py --dataset_config $dataset_config --text_encoder $text_encoder --batch_size $cache_text_encoder_batch_size
accelerate launch --dynamo_backend no --dynamo_mode default --mixed_precision $mixed_precision --num_processes 1 --num_machines 1 --num_cpu_threads_per_process 2 ${MUSUBI_TUNER_PATH}/zimage_train_network.py ${args[@]}